...
public class AvocadoThief {
    public static void main(String[] args) throws IOException{
        ...
        for(int i=0;i<M;i++){
            tk = new StringTokenizer(br.readLine());
            int b1 = Integer.parseInt(tk.nextToken());
            int b2 = Integer.parseInt(tk.nextToken());
    ...
    public static int BFS(int start,int N,int[][]map){
        ...
        while(!queue.isEmpty()){
            ...
            if(currentbuilding==N){
                return mins;
            }
            if(!visited.contains(currentbuilding)){
                visited.add(currentbuilding);
                for(int i=0;i<N;i++){
                    if(map[currentbuilding-1][i]==0){
                        queue.add(i);
                    }
                    else if(map[currentbuilding-1][i]==1){
                        queue.add(i);
                    }
                }
            }
